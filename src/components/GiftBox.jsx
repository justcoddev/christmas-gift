...
const GiftBox = ({ onGiftOpened }) => {
  const handleClick = () => {
    if (onGiftOpened) {
      onGiftOpened(); // Notifica que el botón ha sido clickeado
    }
  };
  return (
    ...
    <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-[150vh] w-[500px] zoom-gift z-10">
      <section className="absolute bottom-0 w-[150px] left-[172px] z-[12] h-[180px] group">
        <button
          onClick={handleClick} // Llama a la función al hacer clic
          className="absolute w-[120px] text-gray-700 font-sans font-extrabold rounded-md shadow-[0_1px_0_1px_#F9564AFF] p-2.5 text-[15px] bg-[#ffa69f] left-0 top-[100px] right-0 mx-auto leading-[26px] z-[-1] text-center transition-all duration-600 ease-in-out delay-[1500ms] group-hover:top-[-10px] group-hover:delay-[400ms]"
        >
          Open Card
        </button>
        {/* aqui debo mandar el permitir abrir lacard */}
        <section className="w-full absolute top-0 transition-all duration-600 ease-in-out delay-[1500ms] group-hover:top-[-100px] group-hover:delay-300">
          ...
          <div className="h-[55px] w-[15px] left-[15px] absolute bg-[#ffb74a] top-[40px] z-[11] after:content-[''] after:border-[9px] after:border-solid after:border-transparent after:border-b-[#ff8168] after:z-[12] after:absolute after:bottom-0 transition-all duration-1000 ease-in-out delay-[1500ms] group-hover:h-8 group-hover:delay-0"></div>
        </section>

        <div className="w-0.5 bottom-0 absolute right-0 left-0 mx-auto bg-[#ffb74a] h-[140px] border-r-[6px] border-l-[6px] border-[#ffb74a] z-10 transition-all duration-1000 ease-in-out delay-[1500ms] group-hover:h-[102px] group-hover:delay-0"></div>
